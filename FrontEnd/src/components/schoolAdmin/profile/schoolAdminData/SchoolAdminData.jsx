...
export default function SchoolAdminData() {
    ...
    const handleFieldChange = (event) => {
      ...
      if (file) {
          ...
          const isValidFileSize = file.size <= 1024 * 1024;
  
          if (!isValidFileType || !isValidFileSize) {
              // إذا كان الملف غير صالح، عيّن رسالة الخطأ باستخدام Formik
              formik.setFieldError('profilePicture', 
                  !isValidFileType ? "نوع الملف غير مدعوم" : "حجم الملف كبير جدًا"
              );
              return  ; // لا تُحدّث previewImage
          }
